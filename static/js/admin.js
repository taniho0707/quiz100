class QuizAdmin {
  ...
  initializeElements() {
    this.elements = {
      ...
      connectionStatusDisplay: document.getElementById(
      ...
      participantCountDisplay: document.getElementById(
  ...
  setupEventListeners() {
    // アクションボタンのイベントリスナー
    this.elements.startEventBtn?.addEventListener('click', () =>
      this.executeAction('start_event')
    );
    this.elements.showTitleBtn?.addEventListener('click', () =>
      this.executeAction('show_title')
    );
    this.elements.assignTeamsBtn?.addEventListener('click', () =>
      this.executeAction('assign_teams')
    );
    this.elements.nextQuestionBtn?.addEventListener('click', () =>
      this.executeAction('next_question')
    );
    this.elements.countdownAlertBtn?.addEventListener('click', () =>
      this.executeAction('countdown_alert')
    );
    this.elements.showAnswerStatsBtn?.addEventListener('click', () =>
      this.executeAction('show_answer_stats')
    );
    this.elements.revealAnswerBtn?.addEventListener('click', () =>
      this.executeAction('reveal_answer')
    );
    this.elements.showResultsBtn?.addEventListener('click', () =>
      this.executeAction('show_results')
    );
    this.elements.celebrationBtn?.addEventListener('click', () =>
      this.executeAction('celebration')
    );

    // デバッグ ステートジャンプ
    this.elements.jumpStateBtn?.addEventListener('click', () =>
      this.handleStateJump()
    );

    // 並び替えボタン
    this.elements.sortByNameBtn?.addEventListener('click', () =>
      this.setSortMode('name')
    );
    this.elements.sortByScoreBtn?.addEventListener('click', () =>
      this.setSortMode('score')
    );
    this.elements.sortByPingBtn?.addEventListener('click', () =>
      this.setSortMode('ping')
    );
    ...
    this.elements.confirmResetDbBtn?.addEventListener('click', () =>
      this.confirmResetDatabase()
    );
    this.elements.cancelResetDbBtn?.addEventListener('click', () =>
  ...
  handleInitialSync(data) {
    ...
    if (data.question_number !== undefined) {
      this.elements.currentQuestion.textContent = data.question_number - 1; // FIXME: 0問目スタートのための暫定対応
    }
  ...
  handleUserJoined(data) {
    if (data.teamname) {
      this.addLog(
        `${data.nickname} が参加しました (${data.teamname}に配置)`,
        'info'
      );
    } else {
  ...
  handleTeamAssignment(data) {
    ...
    this.addLog(
      `チーム分けが完了しました (${data.teams.length}チーム)`,
      'success'
    );
  }
  ...
  updateButtonStates(availableActions) {
    ...
    if (this.elements.assignTeamsBtn) {
      this.elements.assignTeamsBtn.style.display = availableActions.includes(
        'assign_teams'
      )
        ? 'inline-block'
        : 'none';
  ...
  updateEventStatus() {
    ...
    this.elements.currentQuestion.textContent =
      this.currentEvent.question_number - 1 || 0; // FIXME: 0問目スタートのための暫定対応

    this.elements.startEventBtn.disabled =
      this.currentEvent.status === 'started';
    this.elements.nextQuestionBtn.disabled =
      this.currentEvent.status !== 'started';
    this.elements.countdownAlertBtn.disabled =
      this.currentEvent.status !== 'started';
  }
    ...
    sortedParticipants.forEach((user) => {
      ...
      if (pingData) {
        ...
        if (isStale) {
          ...
          const latencyText =
            pingData.result < 0 ? 'timeout' : `${pingData.result}ms`;
          let pingDataStatus = '';
  ...
  updateQuestionDisplay() {
    ...
    let html = `
            <h4>問題 ${this.currentQuestion.question_number - 1}</h4>
            <p><strong>${question.text}</strong></p>
        `; // FIXME: 0問目スタートのための暫定対応
  ...
  updateAnswersDisplay() {
    ...
    const correctCount = Array.from(this.answers.values()).filter(
    ...
    let html = `
            <div class="answer-stats">
                ...
                <div class="answer-stat">
                    <div class="answer-stat-number">${Math.round(
  ...
  updateTeamsDisplay() {
    ...
    const teamsArray = Array.from(this.teams.values()).sort(
      (a, b) => b.score - a.score
    );
  ...
  updateTeamModeDisplay() {
    if (this.elements.teamsContainer) {
      this.elements.teamsContainer.style.display = this.teamMode
        ? 'block'
        : 'none';
    }
    if (this.elements.createTeamsBtn) {
      this.elements.createTeamsBtn.style.display = this.teamMode
        ? 'inline-block'
        : 'none';
    }
  }
  ...
  handleTeamMemberAdded(data) {
    ...
    this.updateTeamsDisplay();
    this.addLog(
      `${data.nickname} が ${data.team_id} に自動配置されました`,
      'success'
    );
  }
  ...
  handlePingResult(data) {
    ...
    if (data.status === 'bad' || data.result > 5000) {
      this.addLog(
        `${data.nickname} の通信が不安定です (${
          data.latency === 0 ? 'タイムアウト' : data.latency + 'ms'
        })`,
        'warning'
      );
    }
  ...
  updateSyncStatusDisplay(syncReport) {
    ...
    if (syncReport.sync_rate !== undefined && syncReport.sync_rate < 80) {
      this.addLog(
        `同期率が低下しています: ${syncReport.sync_rate.toFixed(1)}% (${
          syncReport.synchronized
        }/${syncReport.total_clients})`,
        'warning'
      );
    }
  }
  ...
  async confirmResetDatabase() {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        this.addLog(
          `データベースがリセットされました: ${result.backup_path}`,
          'success'
        );
